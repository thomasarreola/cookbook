...
export const addRecipe = async (db, newRecipe) => {
  try {
    ...
    for (let i = 0; i < newRecipe.ingredients.length; i++) {
      const stockId = await addStock(db, {
        name: newRecipe.ingredients[i][0],
        quantity: 0,
      });
      await addIngredient(db, {
        quantity: newRecipe.ingredients[i][1],
        recipe_id: recipeId,
        stock_id: stockId,
      });
    }
...
export const addStock = async (db, newStock) => {
  try {
    ...
    if (!stockId) {
      const statement = await db.prepareAsync(
        `INSERT INTO stock_list (name, quantity) VALUES(?,?)`,
      );
      ...
      const statement = await db.prepareAsync(
        `UPDATE stock_list SET quantity = quantity+? WHERE id=?`,
      );
...
export const checkStock = async (db, newStock) => {
  try {
    const result = await db.getFirstAsync(
      `SELECT id FROM stock_list WHERE name=?`,
      [newStock.name],
    );
    if (!result) {
      return false;
...
export const addIngredient = async (db, newIngredient) => {
  try {
    const statement = await db.prepareAsync(
      `INSERT INTO ingredient_list (quantity, recipe_id, stock_id) VALUES (?,?,?)`,
    );
    const result = await statement.executeAsync([
